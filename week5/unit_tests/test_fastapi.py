...
def test_health_check():
    ...
    assert response.json() == {
        "Hello": "please add \'/docs\' to ur url and u will be transferred to swagger page with list of request "
...
def test_train():
    ...
    assert response.json() == {'Complete train and sent to wandb with model_name': 'model_random_forest.pkl'}
    assert response.status_code == 200
